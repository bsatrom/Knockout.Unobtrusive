(function(global) {
  ...
  test('value property on binding object binds model property to DOM element with the same id', function() {
    ...
    ko.bindingProvider.instance = ko.announcer.propertyProvider.init(bindings);
  ...
  test('value property with wrapper helper binds model property to DOM elements', function() {
    ...
    var bindings = {
      favoriteColor: ko.announcer.bindingHelper.createWrapper("value", "favoriteColor")
    };
  ...
  test('options binding type can be bound with binding provider', function() {
      var viewModel = {
        languages: ko.observableArray(['JavaScript', 'Ruby', 'C#'])
      };
      ...
      viewModel.languages.push("Smalltalk")

      equal(document.getElementById('languages').options.length, 4);
  }); 

  test('options binding type can be bound with binding helper wrapper', function() {
      var viewModel = {
        languages: ko.observableArray(['JavaScript', 'Ruby', 'C#'])
      };
